 ...
 * @return: null
 */
function articleStudy(aCount,aTime,cCount)
{
 ...
 * @return: null
 */
function videoStudy(vCount,vTime)
{
    h=device.height;//屏幕高
    w=device.width;//屏幕宽
    x=(w/3)*2;//横坐标2分之3处
...
//主函数
function main(aCount,aTime,vCount,vTime,rTime,cCount)
{
    start_app();//启动app
    var start=new Date().getTime();//程序开始时间
    videoStudy(vCount,vTime);//看“百灵”小视频
    listenToRadio();//听电台广播
    var r_start=new Date().getTime();//广播开始时间
    articleStudy(aCount,aTime,cCount);//学习文章，包含点赞、分享和评论
    listenToRadio();//继续听电台
...
ui.all.click(function(){
    ...
    thread=threads.start(function(){
        ...
        main(aCount,aTime,vCount,vTime,rTime,cCount);
    });
});
...
ui.article.click(function(){
    ...
    thread=threads.start(function(){
        ...
        aCount=ui.acount.getText()
        aTime=ui.atime.getText()
        articleStudy(aCount,aTime,cCount);
    });
});
